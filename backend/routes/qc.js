...
router.post('/checks', (req, res) => {
  const { property_id, staff_id, checklist_id, assigned_to_id, date, notes } = req.body;
  let checkId;
  db.exec('BEGIN');
  try {
    const result = db.prepare(`
      INSERT INTO qc_checks (property_id, staff_id, checklist_id, scheduled_by_id, assigned_to_id, date, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(property_id, staff_id, checklist_id, req.manager.id, assigned_to_id, date, notes || '');
